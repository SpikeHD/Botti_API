  ...
  app.get('/followers', async (req, res) => {
    ...
    if (query.uid) {
      const result = await client.query(
        'SELECT u.username, u.email, u.icon, u.bio, u.followers, u.following FROM following f, users u WHERE f.follower=? AND u.uid=f.following',
        [query.uid]
      )
      ...
      const result = await client.query(
        'SELECT u2.username, u2.email, u2.icon, u2.bio, u2.followers, u2.following FROM followers f, users u, users u2 WHERE p.follower = u.uid AND u.username=? AND u2.uid=f.following',
        [query.username]
      )
